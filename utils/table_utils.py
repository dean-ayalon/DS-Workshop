...
def return_unique_values_of_column_from_table(column_name,table_name):
    ...
    #unique_column = pd.DataFrame(table_name[[column_name]])
...
def filter_table_by_unique_ids(filter_column,column_to_filter,table_to_filter):
    filtered_table = pd.concat([chunk[chunk[column_to_filter].isin(filter_column)] for chunk in table_to_filter])
